...
class DataLoader:
    ...
    def __init__(self, det):
        ...
        if self.det in det_list:
            self.det_code = self.det.lower()  # Convert to lowercase for consistency (e.g., 'H1' -> 'h1')
            print(f"Detector selected: {self.det} ({self.det_code})")
        else:
            raise ValueError(f"Invalid detector type: {self.det}. Please choose from {det_list}.")
